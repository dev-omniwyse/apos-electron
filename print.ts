...
export class ShiftsComponent implements OnInit {
        ...
        this.electronService.ipcRenderer.on('paymentsDataResult', (event, data, userID, shiftType) => {
            ...
            if (data != undefined && data.length != 0) {
                this._ngZone.run(() => {
                    ...
                    // }
                    var displayingPayments = this.iterateAndFindUniquePaymentTypeString(this.backendPaymentReport);
                    this.paymentReport = this.generatePrintReceiptForPayments(displayingPayments);
                    //  localStorage.setItem("paymentReceipt", paymentReport)
    ...
    getUsersSalesAndPayments() {
        var shiftStore = JSON.parse(localStorage.getItem("shiftReport"))
        shiftStore.forEach(record => {
            this.electronService.ipcRenderer.send('salesData', Number(record.shiftType), record.openShiftTimeStamp, record.closeShiftTimeStamp, Number(record.userID))
            this.electronService.ipcRenderer.send('paymentsData', Number(record.userID), Number(record.shiftType), record.initialOpeningTime, record.timeClosed, null, null, null)
        });
    ...
    printAllShiftData() {
        ...
        shiftStore.forEach(element => {
            ...
            this.salesPaymentData.forEach(payment => {
        ...
        try {
            // posApplet.printSummaryPaymentsReport(paymentReport);
            this.electronService.ipcRenderer.send("printSummaryPaymentsReport", this.paymentReport);
        } catch (e) {
            console.log("Exception printing payment summary.");
        }
